...
```{r}
covid19_st <- search_tweets("#COVID19 -filter:retweets -filter:quote -filter:replies",n = 18000, include_rts = FALSE, lang = "en") 
...
```{r}
prob1LR <- predict(model1LR,test, type = "response")
# prob1LR
...
As we can see from the confusion matrix, a single decision tree performs significantly better than the logistic regression on such important parameters as accuracy, specificity (precision) and sensitivity (recall). Therefore, there is a lot for potential for random forest approach to improve the predicting power of our model. 
...
```{r warning = FALSE}
caTools::colAUC(cbind(prob1LR,prob2LR,prob1RF), testSet$retwtBi,plotROC = T)
```

Area under the ROC curve can be used to compare various models in order to pick the optimal one and we conclude that random forest model performs the best on this metric as well. 

From the ROC curve we can see that Logistic Regression model 2 (LR2) (fitted only on followers, mentions and hashtags) performs much better compared to LR1 model (fitted on all data). We can use cross-validation to try fitting different logistic regression models and seeing which one performs the best:  
...
First I changed the retweets into 5 CATegories that define how "popular" the tweet is: 
...
While other possible divisions exist, I wanted to make sure that there are enough samples in each category. 
```{r}
...
```{r}
modelMLR_A <- multinom(retwtCAT ~ followers + friends + score + mentions + hashtags, data = afinn_RK)
summary(modelMLR_A)$AIC
```
```{r}
modelMLR_F <- multinom(retwtCAT ~ followers, data = afinn_RK)
summary(modelMLR_F)$AIC

modelMLR_FR <- multinom(retwtCAT ~ friends, data = afinn_RK)
summary(modelMLR_FR)$AIC

modelMLR_S <- multinom(retwtCAT ~ score, data = afinn_RK)
summary(modelMLR_S)$AIC

modelMLR_M <- multinom(retwtCAT ~ mentions, data = afinn_RK)
summary(modelMLR_M)$AIC

modelMLR_H <- multinom(retwtCAT ~ hashtags, data = afinn_RK)
summary(modelMLR_H)$AIC
```
...
```{r}
predRK_MLR <- predict(modelRK_MLR,testRK, type = "terms")
predRK_MLR
confusionMatrix(predRK_MLR, testRK$retwtCAT)
```
